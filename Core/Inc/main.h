/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define SW2_Pin GPIO_PIN_1
#define SW2_GPIO_Port GPIOA
#define PWMA_Pin GPIO_PIN_5
#define PWMA_GPIO_Port GPIOA
#define ADIR2_Pin GPIO_PIN_6
#define ADIR2_GPIO_Port GPIOA
#define ADIR1_Pin GPIO_PIN_7
#define ADIR1_GPIO_Port GPIOA
#define STBY_Pin GPIO_PIN_4
#define STBY_GPIO_Port GPIOC
#define BDIR1_Pin GPIO_PIN_5
#define BDIR1_GPIO_Port GPIOC
#define BDIR2_Pin GPIO_PIN_0
#define BDIR2_GPIO_Port GPIOB
#define PWMB_Pin GPIO_PIN_2
#define PWMB_GPIO_Port GPIOB
#define LED1_Pin GPIO_PIN_12
#define LED1_GPIO_Port GPIOB
#define LED2_Pin GPIO_PIN_14
#define LED2_GPIO_Port GPIOB
#define S_ON_Pin GPIO_PIN_6
#define S_ON_GPIO_Port GPIOC
#define S1_Pin GPIO_PIN_7
#define S1_GPIO_Port GPIOC
#define S2_Pin GPIO_PIN_8
#define S2_GPIO_Port GPIOC
#define S3_Pin GPIO_PIN_9
#define S3_GPIO_Port GPIOC
#define S4_Pin GPIO_PIN_8
#define S4_GPIO_Port GPIOA
#define S5_Pin GPIO_PIN_9
#define S5_GPIO_Port GPIOA
#define S6_Pin GPIO_PIN_10
#define S6_GPIO_Port GPIOA
#define S7_Pin GPIO_PIN_11
#define S7_GPIO_Port GPIOA
#define S8_Pin GPIO_PIN_12
#define S8_GPIO_Port GPIOA

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
